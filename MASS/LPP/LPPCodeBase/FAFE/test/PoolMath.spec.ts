...
/** POOL does transferFrom(...) inside FAFEPool.supplicate → approve the POOL (and optionally Router) */
async function fundAndApproveForSupplicate(opts: {
  token: TestERC20;
  minter: any; // deployer (has mint perms in TestERC20)
  payer: any;
  pool: FAFEPool;
  router?: FAFERouter; // optional
  amount: bigint;
}) {
...
describe("Pool math integrity", () => {
  ...
  describe("Validation & slippage", () => {
    ...
    it("reverts on zero amountIn", async () => {
      ...
      ).to.be.reverted; // "zero" from FAFEPool (modifier)
    });
  });
  ...
  describe("Offset multiplier behavior", () => {
    ...
    it("USDC->ASSET with -5000 bps returns 50% premium", async () => {
      ...
      const { a: reserveAsset, u: reserveUsdc } = await reserves(pool as FAFEPool);

...
contract FAFERouter is IFAFERouter {
    ...
    function setOrbit(address startPool, address[] calldata pools_) external onlyTreasury {
        ...
        address a0 = IFAFEPool(pools_[0]).asset();
        address u0 = IFAFEPool(pools_[0]).usdc();
        for (uint256 i = 1; i < pools_.length; i++) {
            require(IFAFEPool(pools_[i]).asset() == a0 && IFAFEPool(pools_[i]).usdc() == u0, "orbit: mismatched pair");
        }
    ...
    function setDualOrbit(
        ...
        for (uint256 i = 1; i < neg.length; i++) {
            require(IFAFEPool(neg[i]).asset() == a0 && IFAFEPool(neg[i]).usdc() == u0, "dual: NEG mismatch");
            require(IFAFEPool(pos[i]).asset() == a0 && IFAFEPool(pos[i]).usdc() == u0, "dual: POS mismatch");
        }
        require(IFAFEPool(pos[0]).asset() == a0 && IFAFEPool(pos[0]).usdc() == u0, "dual: POS mismatch");
        ...
        returns (uint256 totalOut)
    {
        ...
        unchecked {
            for (uint256 i = 0; i < orbit.length; i++) {
                ...
                uint256 out = IFAFEPool(pool).supplicate(
                    payer,
                    address(this),
                    assetToUsdc,
                    p.amountIn,
        ...
        returns (address[] memory orbit, bool assetToUsdc, address tokenIn, address tokenOut)
    {
        ...
        require(orbit.length > 0, "empty orbit");
        address a = IFAFEPool(orbit[0]).asset();
        address u = IFAFEPool(orbit[0]).usdc();
    ...
    function _takeInputFeeAndDonate(
        ...
        if (poolsFt > 0) {
            ...
            bool isUsdc = (tokenIn == IFAFEPool(pool).usdc());
            IFAFEPool(pool).donateToReserves(isUsdc, poolsFt);
        }

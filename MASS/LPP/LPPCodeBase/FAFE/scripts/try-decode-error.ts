...
async function main() {
  ...
  const manifest = JSON.parse(fs.readFileSync("test/Deployment/deployment-manifest.json", "utf-8"));
  ...
  const TreasuryFactory = await ethers.getContractFactory("FAFETreasury");
  ...
  const RouterFactory2 = await ethers.getContractFactory("FAFERouter");

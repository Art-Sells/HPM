...
const SNAPSHOT_DIR = path.resolve(__dirname, "../test/MEV/test/__snapshots__");
...
const poolManifestPath = path.resolve(__dirname, "../pool-manifest.json");
...
async function main() {
  ...
  const factory = await ethers.getContractAt("LPPFactory", manifest.contracts.LPPFactory);

  const access = await ethers.getContractAt(
    "LPPAccessManager",
    manifest.contracts.LPPAccessManager,
    treasuryOps
  );

  const treasury = await ethers.getContractAt(
    "LPPTreasury",
    manifest.contracts.LPPTreasury,
    treasuryOps
  );

  const router = await ethers.getContractAt(
    "LPPRouter",
    manifest.contracts.LPPRouter,
    massTester
  );
  ...
  poolManifest.pools[poolKey] = {
    ...
    router: manifest.contracts.LPPRouter,
  };

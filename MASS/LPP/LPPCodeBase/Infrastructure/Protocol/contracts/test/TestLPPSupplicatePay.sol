...
contract TestLPPSwapPay is ILPPSwapCallback {
    ...
    function lppSwapCallback(
        ...
        if (pay0 > 0) {
            IERC20Minimal(ILPPPool(msg.sender).token0()).transferFrom(sender, msg.sender, uint256(pay0));
        } else if (pay1 > 0) {
            IERC20Minimal(ILPPPool(msg.sender).token1()).transferFrom(sender, msg.sender, uint256(pay1));
        }
    }
}

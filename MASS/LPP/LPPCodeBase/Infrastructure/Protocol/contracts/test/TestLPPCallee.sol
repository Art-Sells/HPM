...
contract TestLPPCallee is ILPPMintCallback, ILPPSupplicateCallback, ILPPFlashCallback {
    ...
    function lppMintCallback(
        ...
        if (amount0Owed > 0)
            IERC20Minimal(ILPPPool(msg.sender).token0()).transferFrom(sender, msg.sender, amount0Owed);
        if (amount1Owed > 0)
            IERC20Minimal(ILPPPool(msg.sender).token1()).transferFrom(sender, msg.sender, amount1Owed);
    }
    ...
    function flash(
        ...
        ILPPPool(pool).flash(recipient, amount0, amount1, abi.encode(msg.sender, pay0, pay1));
    }

    function lppFlashCallback(
        uint256 fee0,
        uint256 fee1,
        bytes calldata data
    ) external override {
        ...
        if (pay0 > 0) IERC20Minimal(ILPPPool(msg.sender).token0()).transferFrom(sender, msg.sender, pay0);
        if (pay1 > 0) IERC20Minimal(ILPPPool(msg.sender).token1()).transferFrom(sender, msg.sender, pay1);
    }
}

    ...
    function lppSwapCallback(
        ...
        if (pools.length == 1) {
            ...
            address tokenToBePaid =
                amount0Delta > 0 ? ILPPPool(msg.sender).token0() : ILPPPool(msg.sender).token1();
            ...
            if (amount0Delta > 0) {
                IERC20Minimal(ILPPPool(msg.sender).token0()).transferFrom(
                    payer,
                    msg.sender,
                    uint256(amount0Delta)
                );
            } else {
                IERC20Minimal(ILPPPool(msg.sender).token1()).transferFrom(
                    payer,
                    msg.sender,

...
library PoolTicksCounter {
    ...
    /// direction of the supplicate. If we are supplicating upwards (tickAfter > tickBefore) we don't want to count tickBefore but we do
    /// want to count tickAfter. The opposite is true if we are supplicating downwards.
    function countInitializedTicksCrossed(
        ...
        {
            // Get the key and offset in the tick bitmap of the active tick before and after the supplicate.
            ...
            // In the case where tickAfter is initialized, we only want to count it if we are supplicating downwards.
            // If the initializable tick after the supplicate is initialized, our original tickAfter is a
            // multiple of tick spacing, and we are supplicating downwards we know that tickAfter is initialized
            ...
            // In the case where tickBefore is initialized, we only want to count it if we are supplicating upwards.
